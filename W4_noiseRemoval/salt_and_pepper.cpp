...
Mat Salt_pepper_noise_removal_Gray(const Mat input, int n, const char *opt) {
	...
	median = floor(kernel_size *kernel_size/ 2 +0.5);
	float tempa;
	float tempb;

	// initialize median filter kernel
	Mat kernel = Mat::zeros(kernel_size * kernel_size, 1, CV_32F);
	...
	for (int i = 0; i < row; i++) {
			...
			if (!strcmp(opt, "zero-padding")) {
				...
				for (int x = -n; x <= n; x++) { // for each kernel window
					for (int y = -n; y <= n; y++) {
						...
						if ((i + x <= row - 1) && (i + x >= 0) && (j + y <= col - 1) && (j + y >= 0)) {
							kernel.at< float>(count++,0) = (float) input.at<G>(i+x, j+y) ;
							
						}
						//else { kernel.at< float>(count++, 0) = 0; }
					}
				}
			}
				...
				for (int x = -n; x <= n; x++) { // for each kernel window
					for (int y = -n; y <= n; y++) {
						...
						kernel.at<float>(count++,0) = input.at<G>(tempa, tempb);
				...
				for (int x = -n; x <= n; x++) { // for each kernel window
					for (int y = -n; y <= n; y++) {
						...
						if ((i + x <= row - 1) && (i + x >= 0) && (j + y <= col - 1) && (j + y >= 0)) {
							kernel.at<float>(count++,0) = input.at<G>(i + x, j + y);
		...
		output.at<G>(i, j) = kernel.at<float>(median+1, 0);
...
Mat Salt_pepper_noise_removal_RGB(const Mat input, int n, const char *opt) {
	...
	for (int i = 0; i < row; i++) {
		for (int j = 0; j < col; j++) {
			...
			if (!strcmp(opt, "zero-padding")) {
				...
				for (int x = -n; x <= n; x++) { // for each kernel window
					for (int y = -n; y <= n; y++) {
						...
						if ((i + x <= row - 1) && (i + x >= 0) && (j + y <= col - 1) && (j + y >= 0)) {
							kernel.at< float>(count, 0) = input.at<C>(i + x, j + y)[0];
							kernel.at< float>(count, 1) = input.at<C>(i + x, j + y)[1];
							kernel.at< float>(count, 2) = input.at<C>(i + x, j + y)[2];
						}
						count++;
					}
				...
				for (int x = -n; x <= n; x++) { // for each kernel window
					for (int y = -n; y <= n; y++) {
						...
						kernel.at<float>(count,0) = input.at<C>(tempa, tempb)[0];
						kernel.at<float>(count,1) = input.at<C>(tempa, tempb)[1];
						kernel.at<float>(count,2) = input.at<C>(tempa, tempb)[2];
			...
			output.at<C>(i, j)[0] = kernel.at<float>(median, 0);
			output.at<C>(i, j)[1] = kernel.at<float>(median, 1);
			output.at<C>(i, j)[2] = kernel.at<float>(median, 2);
		}
	}
